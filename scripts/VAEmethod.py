...
def do_it(data, eps):
    ...
    for i in range(r):
        ...
        if i == r - 1:
            ...
            ax2 = ax1.twinx()
            ax1.plot(np.arange(len(loss_history)) / 1000, loss_history, "r-") 
            ax2.plot(np.arange(len(acc_history)) / 1000, acc_history, "b-")  
            ...
            ax3.set_title(r"$\mu$ and $\sigma$ distribution in $Z$")
            ax3.set_xticklabels([])
            ax3.set_xticks([])
            ax3.set_yticklabels([])
            ax3.set_yticks([])
